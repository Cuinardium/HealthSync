...
const VacationCard: React.FC<VacationCardProps> = ({
  ...
  const color = vacation.isHappening
    ? "success"
    : vacation.isPast
      ? "secondary"
      : "primary";
  ...
  return (
    <Card
      className="shadow-sm"
      border={vacation.isHappening ? "success" : "light"}
    >
      <Card.Body>
        <Row className="align-items-center">
          ...
          <Col md={2} sm={3}>
            <h5 className="text-center">{fromDate}</h5>
            <div className="text-center text-muted"> {vacation.fromTime}</div>
          </Col>
          <Col md={1} sm={1}>
            <FaArrowRight className={`text-${color} text-center`} />
          </Col>
          <Col md={2} sm={3}>
            <h5 className="text-center">{toDate}</h5>
